...
WORLD_CONF_DIR="$HERE/world-conf"
...
GENERATED_MARK="GENERATED by render-assets.sh"
...
render() { # <template file> <output file>
    ...
    # Safety net: never silently destroy a hand-maintained file. Every template
    # carries $GENERATED_MARK, so an output file without it was written by hand
    # and is about to be clobbered. Keep a timestamped copy and say so loudly.
    if [ -f "$2" ] && ! grep -q "$GENERATED_MARK" "$2"; then
        local bak="$2.handedited-$(date +%Y%m%d-%H%M%S)"
        cp -a "$2" "$bak"
        echo "   WARNING: $(basename "$2") was hand-edited, not generated." >&2
        echo "            Saved a copy at $bak before overwriting." >&2
        echo "            Move those settings to world-conf/${WORLD:-<id>}/$(basename "$2")" >&2
        echo "            so they survive future re-renders (see SETUP.md)." >&2
    fi
...
# ---- per-world hand-maintained conf ----
# The four files above are rendered from ONE shared template, so hand-editing the
# rendered output is destroyed on the next re-render. Put per-world settings in
# production/world-conf/<id>/ instead — tracked in git, applied here.
#
# For the four rendered confs the file is APPENDED, so it wins without losing the
# env-driven values above it: battle_config_read / map_config_read assign as they
# read line by line, so the last occurrence of a key takes effect.
#
# Any other filename is COPIED in. That is how you supply the conf/import files
# this script does not render (groups.yml, atcommands.yml, log_conf.txt, …) —
# the mount replaces all of conf/import, so the image's copies are invisible.
WORLD_CONF_SRC="$WORLD_CONF_DIR/$WORLD"
if [ -d "$WORLD_CONF_SRC" ]; then
    for src in "$WORLD_CONF_SRC"/*; do
        [ -e "$src" ] || continue          # unmatched glob in an empty dir
        base="$(basename "$src")"
        case "$base" in
            inter_conf.txt|char_conf.txt|map_conf.txt|battle_conf.txt)
                {
                    printf '\n// ---------------------------------------------------------\n'
                    printf '// Appended from world-conf/%s/%s — overrides the above.\n' "$WORLD" "$base"
                    printf '// ---------------------------------------------------------\n'
                    cat "$src"
                } >> "$CONFIG_DIR/$base"
                echo "   appended world-conf/$WORLD/$base -> $CONFIG_DIR/$base"
                ;;
            *)
                cp -a "$src" "$CONFIG_DIR/$base"
                echo "   copied   world-conf/$WORLD/$base -> $CONFIG_DIR/$base"
                ;;
        esac
    done
fi
